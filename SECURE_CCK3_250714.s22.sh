...
get_via_sftp() {
  ...
  if [ $SFTP_RC -ne 0 ] && [ "$REMOTE_SSH_PORT" != "22" ]; then
    say "[WARN] SFTP failed on discovered port $REMOTE_SSH_PORT. Trying default port 22 ..."
    REMOTE_SSH_PORT=22
    run_sftp
  fi

  if [ $SFTP_RC -ne 0 ]; then
    die "Could not establish SFTP connection (port $REMOTE_SSH_PORT)."
  fi
  ...
  if [ -s "$WHOIS_OUT" ]; then
    ...
    die "SFTP download failed or whois file is empty: $WHOIS_OUT"
  fi
}
